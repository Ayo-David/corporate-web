    ...
    <>
      ...
      {data.map((benefits: any, ind: number) =>
        <React.Fragment key={ind}>
          ...
          <div className="benefits">
            <div className="container">
              <div className={`section flex ${ind % 2 === 0 ? 'flex-md-row-reverse' : 'flex-md-row'} flex-column`}
                   key={ind}>
          ...
          <div className="ready-to-join flex flex-column">
            <div className="title">{benefits.attributes.field_heading}</div>
            <div className="content">
              <div className="row flex-row">
                {benefits.readyToJoin.map((item: any, i: number) =>
                  <div className="col-md-6 col-sm-12 align-items-center d-flex flex-column text-link" key={i}>
                    <div className="text">{item.attributes.field_description}</div>
                    <NavLink to={'/'} className="btn btn-black">{item.attributes.field_single_link.title}</NavLink>
                  </div>
                )}
              </div>
            </div>
