...
interface IBussinessCurrentAccountClassicProps {
...
const BussinessCurrentAccountClassic: React.FunctionComponent<IBussinessCurrentAccountClassicProps> =
  (props) => {
    ...
    useEffect(() => {
      ...
      nprogress.start();
      props.dataAction.getBusinessCurrentAccountsContentData();
    ...
    const getContentTemp = (item: any, index: number) => {
      switch (item.type) {
        ...
        case 'paragraph--customer_interest_links':
          return <RelatedProductsAndServicesSingle dataList={item} />;
