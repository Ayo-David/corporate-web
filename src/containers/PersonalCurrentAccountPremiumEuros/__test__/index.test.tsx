...
import PersonalCurrentAccountPremiumEuros from '../index';
...
describe('PersonalCurrentAccountPremiumEuros testing', () => {
  ...
  const createWrapper = (data: any)=>mount(
    <AppContainer>
      <PersonalCurrentAccountPremiumEuros {...data} />
    </AppContainer>
  )

  let mockGetPersonalCurrentAccountsEurosContentData:any, mockGetHeaderMenuData: any, mockGetFooterData: any,
  ...
  beforeEach(() => {
    jest.useFakeTimers();
    mockGetPersonalCurrentAccountsEurosContentData = jest.spyOn(dataSvc, 'getPersonalCurrentAccountsEurosContentData').mockResolvedValue({
      data: {
        id: 'id',
        attributes: {
          body: {
  ...
  it('Should render elements correctly', async() => {
    ...
    expect(mockGetPersonalCurrentAccountsEurosContentData).toHaveBeenCalledTimes(1)
